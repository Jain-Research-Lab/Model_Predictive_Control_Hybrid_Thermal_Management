...
%               Di0 - seed for newton method of matrix inverse
...
function [xn,phi,Di,dXdU] = Hybrid_TMS_Model_EXP_Simple(x,u,d,vTMS,Di0,E)
%% State Space Matrix
[Mi, A, d] = Hybrid_TMS_Model_SS_Simple(x,u,d,vTMS);
At = (vTMS.dt.*Mi).*[A d; vTMS.Z];

%% Low Precision Maxtrix Exponential
% odd and even
b = vTMS.b_EXPM;
U = b(2)*At;
V = b(1)*vTMS.Iuv;

% inaccurate matrix inversion
Di = Di0*((2*vTMS.Iuv) + ((U - V)*Di0));
eAB = (Di*(U + V));
