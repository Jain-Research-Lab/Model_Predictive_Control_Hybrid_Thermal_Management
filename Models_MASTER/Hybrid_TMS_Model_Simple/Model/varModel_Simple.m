...
%% Hybrid TMS Class
classdef varModel_Simple < handle
...
%% Hybrid TMS Methods
    methods
        ...
        function [varTMS, vTMS] = varModel_Simple()
            varTMS = set_simulation_timing(varTMS);
            varTMS = set_system_discretization(varTMS);
        ...
        function varTMS = set_system_discretization(varTMS)
        % HYBRID TMS SYSTEM DISCRETIZATION  
            % Overall Hybrid TMS System Configuration [Fundamental]
            varTMS.nTMS = 5; % number of states in non-hybrid TMS Model
            varTMS.nms  = 1; % number of TES modules in series
            varTMS.nmp  = 1; % number of TES modules in parallel
            ...
            varTMS.nh  = 1;  % per module number of composite (hexadecane + aluminum) control volumes in y-direction
            varTMS.dnx = 1;  % per module number of control volumes in x-direction
        ...
        % TES MODULE GEOMETRY    
            ...
            % Material Lengths [Fundamental]
            varTMS.Lx = 0.15*4;    % length of the TES Module x-direction [m]
            ...
            % Fin Counts [Fundamental]
            varTMS.nf_h = 4*75; % number of fins submerged in hexadecane per module
            varTMS.nf_w = 4*78; % number of fins submerged in water per module
        ...
        function varTMS = compute_Su_E_matrix(varTMS)
            ...
            nK = length(compute_weight_vector_Simple([0;0],zeros(nk,1),varTMS));
            ...
            for i = 1:varTMS.nu
                ...
                Kj = compute_weight_vector_Simple(u_grad_in(i,:)',zeros(nk,1),varTMS);
                K = zeros(nK,1);
                K(nM:end) = Kj(nM:end);
        ...
        function varTMS = construct_TMS_graph(varTMS)
            ...
            % Construct Hybrid TMS Undirected Edges
                tu = [2; 4; 6; 7];
                su = [3; 5; 7; 8];
                varTMS.Gu = graph(tu,su);
                                
            % Construct Hybrid TMS Directed Edges
                td = [1;3;5;5;6];
                sd = [3;5;1;6;1];
