...
%               UVinv_0 - seed for newton method of matrix inverse
%               E - the gradient matrix dU/dx
%               Iuv - n by n identity matrix
%               Z - n+1 row vector of zeros
% 
% Return     :  xn - states after dt interval
%               phi - temperature gradient matrix dxn/dx
%               invUV - next time step seed of newton method of matrix inverse
...
function [Mi, A, d] = Hybrid_TMS_Model_SS_Simple(x,u,d,vTMS)
%% Compute Material Properties
[k_hex,cp_hex] = compute_hex_properties_Simple(x(vTMS.n_hex_start:end), vTMS);
...
K = compute_weight_vector_Simple(u,k_hex,vTMS);
