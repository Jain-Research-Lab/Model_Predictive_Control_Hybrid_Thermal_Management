...
function [J, dJ] = get_J_EXP(x0,U0,d_MPC,vTMS,vMPC)
%% Constants
P = vMPC.Npx;
ns = vTMS.ns;
nx = vTMS.nx;
...
[J,dJ] = obj_EXP(U0(:),x0,d_MPC,Ru,Ru_m,Rdu,Rdu_m,R_dJ_dU,P,nx,nu,vMPC,C,vTMS,E,dJdy,Z2);

%% Cost Function Definition
    function [J,dJ] = obj_EXP(U0,x0,d_MPC,Ru,Ru_m,Rdu,Rdu_m,RdJ_dU,P,nx,nu,vMPC,C,vTMS,E,dJdy,Z2)
        ...
        [x0, ~, Su] = Hybrid_TMS_Model_EXPM(x0,ui,di,vTMS,E);
        ...
        for i = 1:P
            % Extract Control Actions for the current time step
            ui = U0((i-1)*vMPC.nu+1:i*vMPC.nu);

            % Extract Disturbances for the current time step
            di = d_MPC(:,i);

            % evolve system for dt seconds
            [x0, Sx, Su] = Hybrid_TMS_Model_EXPM(x0,ui,di,vTMS,E);
