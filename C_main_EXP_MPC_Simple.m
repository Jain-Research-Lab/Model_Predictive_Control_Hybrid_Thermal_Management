...
[varTMS_MPC, vTMS_MPC]  = varModel_Simple;              % define TMS variable class
...
[varNMPC_MPC, vMPC_MPC]   = varController_Simple(varTMS_MPC); % define Controller variable class

%% Load High Dimension MPC Parameter Set
[varNMPC_SIM, vMPC_SIM]   = varController(varTMS_SIM); % define Controller variable class

%% Model Definition
fmpc  = @MPC_EXP_Simple;                % controller function
fsim  = @Hybrid_TMS_Model_ODE;          % ground truth for states
fout_MPC  = @outputs_Simple;            % output for MPC
fout_SIM  = @outputs;                   % output for SIM
fcon  = @Hybrid_TMS_Lumped_Inverse_SE_Simple;  % convert sim to mpc model
...
optionsMPC = optimoptions('fmincon','Algorithm','sqp','Display','none', ...
	...
	'ConstraintTolerance',2e-3,'OptimalityTolerance',1e-3,'StepTolerance',1e-3, ...
...
save("Controllers_MASTER/Data/MPC_EXP_Simple.mat","data")
